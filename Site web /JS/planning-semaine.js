...
function deposerRecette(cellule) {
    if (recetteSelectionnee) {
        cellule.innerHTML = `<p>${recetteSelectionnee}</p>`;
...
const prixAliments = {
    ...
    "Poivron": 0.6, "Laitue": 0.6, "Chou": 0.4, "Asperge": 1.0,
    "Oignons": 0.6, "Fenouil": 1.0, "Patate douce": 0.6, "Riz": 1.0,
    "Blé": 0.8, "Quinoa": 3.0, "Pâtes": 1.0, "Pomme de terre": 0.4,
    ...
    "Crevettes": 10.0, "Champignons": 1.0
};
...
function genererListeCourses() {
    ...
    document.querySelectorAll("#planning td").forEach(cell => {
        const recetteNom = cell.textContent.trim();
    ...
    for (const ingredient in courses) {
        ...
        // Calcul du prix en appliquant une réduction de 40% si la quantité est inférieure à 100g
        let prixTotal;
        if (quantiteTotale < 100) {
            prixTotal = (quantiteTotale / 100) * prixPar100g * 0.6; // Applique 60% du prix
        } else {
            prixTotal = (quantiteTotale / 100) * prixPar100g;
        }
